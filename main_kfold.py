...
class KNN:
	def getKNeighbors(self, home): return sorted(zip(self.X_train, self.y_train), key=lambda neighbor: euclideanDistance(home, neighbor[0]))[:5]
	...
	def fit(self, X_train, y_train): self.X_train, self.y_train = X_train, y_train

	def predict(self, X_test): return array([self.getPrediction(self.getKNeighbors(home)) for home in X_test])
